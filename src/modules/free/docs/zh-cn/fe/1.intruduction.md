...
FreeEIS中，任何的功能都应该被实现为独立的模块，而这些独立的模块通常都是可以独立运行的，开发人员可以自行开发调试，而不需要依赖于其他部分。而负责集成的人员，只需要将各功能模块关联到一个统一的集成环境中，根据需要对每个功能模块进行配置，即可运行起整套系统。

但FreeEIS也实现了模块对模块的扩展功能，以满足“模块插件”的需求，这种情况下，新开发的“插件”模块需要在被扩展的模块存在的情况下进行。在此种情况下，模块不能真的完全独立开发，但考虑到新模块是某现有模块的扩展插件，此两个模块可以做为一个整体考虑的情况下，我们依然可以说他是独立开发的。

这是FreeEIS开发的第一核心思想。

## 前提条件

FreeEIS前端基于Quasar Framework开发而来，所以您需要对Quasar Framework有一定的了解，而Quasar Framework是一个基于VUE的前端框架，包括组件库和跨平台支持等特性，所以您也需要对VUE有更深的了解。当前版本中，我们使用了Quasar Framework 2.x，其基于VUE3.x。

为了使用FreeEIS开发应用，您还可能需要了解Webpack和NodeJS本身的一些内容。但是因为VUE和Quasar Framework都进一步做了封装，所以在一般情况下，您可能不需要意识到他们的存在。

## 脚手架

FreeEIS前端开发通过FreeEIS脚手架工程开始。FreeEIS前端脚手架工程基于Quasar Framework的脚手架工程，只是把我们内核模块嵌入，进而实现了FreeEIS模块的加载和运行工作。
...
此时可以通过http://localhost:8080来查看系统页面，但请注意终端输出，在当端口8080已经被占用时，可能会使用8081等其他端口。
...
在这个目录结构中，有很多是Quasar Framework所定义的，请参考官方文档，我们只介绍在开发FreeEIS应用时直接相关的部分：
...
- `src`，这里是开发代码的存放目录。
  ...
  - `config`，配置文件存放位置，用于配置FreeEIS中不能运行环境下的全局配置信息，也可以用以覆盖各模块的默认配置以达到定制使用模块的目标。默认我们放入了三个环境的配置文件，但您完全可以根据自己的需要扩充。
    - `config.default.js`，默认配置，所有环境公用的配置信息，或默认的配置信息。
  ...
  - `css`，全局样式文件保存的位置。我们建议使用FreeEIS的样式皮肤模块，而尽量减少在此处定义全局样式。但基于显而易见的原因，我们保留这里的功能。
    - `app.scss`，全局样式入口。
...
这里，我将页面、路由、模块入口都写在了同一个文件当中，这可以运行，但从通常的最佳实践来说，我们应该把他们拆分开来。FreeEIS对于如何在模块内部拆分组织代码没有强制性规范，您可以根据自己的习惯来进行开发，只要在模块入口文件中指明应该的路由即可。
...
上面的拆分是示意性的，在复杂的模块中，需要根据实际情况来定，但核心思路是，FreeEIS不限制您的代码组织方式，只要确保在模块入口文件中可以找到相应的路由即可。


### 模块配置

组件或页面有props可以使我们的使用者对某些内容或逻辑加以控制，模块也同样需要这样的机制。

在我们的这个实例模块中，假设我们需要say hello的目标不是固定的'world'， 而是需要根据情况而定，那么我们可以在模块中添加相应的配置参数，使得这部分内容可变。
...
如果不使用组合式API，你可以通过`this.ctx`访问全局上下文，其中包括如下两个重要的内容：
...
上面我们看到，通过`vm.postRequest`我们调用了一个`post`接口，其实在`vm`(也就是非组合式API情况下的`this`)中，我们有如下方法：
...
如果我们使用内置的`free-fe-core-modules`模块，则可以直接使用它的`SelectLocales`组件，如下：
...
`SelectLocales`组件默认显示一个图标，当点击时出现下来菜单来选择语言，此组件更适合放在像顶部菜单栏之类的位置。而且，当我们有更多的语言需要支持时，只要在i18n目录中添加相应的翻译字典，即可自动添加到下拉菜单中，实现实时翻译。比如，我们添加另一种语言，如下：
...
但，因为火星人不认识中文`火星语`这三个字，所以，虽然我们地球人对火星语的定义为`火星语`，就像我们定义中文为`zh-cn`一样，那么我们需要在页面上的下拉菜单中显示火星语的`火星语`的意思，此时我们需要在全局配置菜单中的`locales`是添加此项，比如下：
